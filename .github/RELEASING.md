...
1. 在 GitHub 创建 6 个 **public** 仓库，保持默认分支 `main`：
   `cmf-core`、`cmf-module-users`、`cmf-module-roles`、`cmf-module-auditing`、`cmf-module-media`、`cmf-module-import`。
...
3. 推送首个版本 tag，等 workflow 跑完（6 个包各自打一个，互不影响）：
   ...
   ```bash
   git tag core-v1.0.0 users-v1.0.0 roles-v1.0.0 auditing-v1.0.0 media-v1.0.0 import-v1.0.0
...
- monorepo tag 必须是 `{package}-v{version}` 格式（package 为 `core/users/roles/auditing/media/import` 之一），
